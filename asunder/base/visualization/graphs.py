...
def draw_network_with_labels(
    ...
    G : nx.Graph
        NetworkX graph.
    community_map_labels : dict[str, int]
...
def draw_colored_constraint_graph(
    ...
    G : nx.Graph
        NetworkX graph.
    node_type : dict
        Mapping from node to node type.
    edge_type : dict
        Mapping from edge to edge type.
    color_nodes_by_type : bool
        Color nodes by node type if `True`.
    color_edges_by_type : bool
        Color edges by edge type if `True`.
    node_homog_color : str
        Color code for homogeneously colored nodes.
    edge_homog_color : str
        Color code for homogeneously colored edges.
    linear_color : str
        Color code for linear nodes.
    nonlinear_color : str
        Color code for nonlinear nodes.
    cont_color : str
        Color code for continuous edges.
    int_color : str
        Color code for integer edges.
    """
    import matplotlib.pyplot as plt
    import networkx as nx
