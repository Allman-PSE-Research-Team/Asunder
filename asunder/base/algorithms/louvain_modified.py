...
def directed_to_undirected(A: sparse.csr_matrix, average: bool = True) -> sparse.csr_matrix:
    ...
    A : sparse.csr_matrix
        Input adjacency.
    average : bool
        Enable / disable returning the average. Defaults to ``True``.
...
# TODO: Normalize references to degree matrix (a,k), total weight m vs 2m vs mprime, and dual matrices.
...
class ModifiedLouvain:
    ...
    Attributes
    ----------
    labels_ : np.ndarray of int, shape (N,)
    ...
    def fit(self,
        ...
        Parameters
        ----------
        input_matrix : array-like or sparse, shape (N, N)
    ...
    def fit_standard_louvain(self, input_matrix) -> "ModifiedLouvain":
        ...
        Parameters
        ----------
        input_matrix : sequence[sequence[int or float]]
            Input sequence; typically adjacency like.
    ...
    def fit_modified_one_level(self,
        ...
        This updates ``labels_`` and ``probs_`` on the original nodes and
        stores ``obj_val_`` when dual scalars are provided.
        
        Parameters
        ----------
        input_matrix : sequence[sequence[int or float]]
            Input sequence; adjacency-like.
        duals : Dict[str, np.ndarray or float]
            Dual terms used to modify the community detection objective. 2D, 1D and scalar dual values are supported.
            Missing entries are treated as zeros.
        a : Optional[np.ndarray], shape (N,)
    ...
    def _modularity(A: sparse.csr_matrix, labels: np.ndarray, k: np.ndarray, m: float, gamma: float) -> float:
        ...
        labels : np.ndarray of int, shape (N,)
            Community assignment vector.
        k : np.ndarray of int | float, shape (N,)
            Degree vector.
        m : float
            Total graph weight, computed as sum(A) / 2.
        gamma : float
    ...
    def _assemble_S(duals: Dict[str, np.ndarray | float], n: int) -> np.ndarray:
        ...
        Returns
        -------
        S : np.ndarray of float, shape (N, N)
            Aggregated dual matrix.
        """
    ...
    def _build_M(A: sparse.csr_matrix,
        ...
        A : sparse.csr_matrix, shape (N, N)
            Input adjacency / weight matrix.
        a : np.ndarray of int or float, shape (N,)
            Degree-like vector; defaults to row sums of the symmetrized adjacency.
        mprime : float
            Sum of degrees; defaults to sum(a).
        gamma : float
    ...
    def _objective_from_partition(Mmat: np.ndarray, labels: np.ndarray) -> float:
        ...
        Parameters
        ----------
        Mmat : np.ndarray of float, shape (N, N)
            Modified modularity matrix.
        labels : np.ndarray of int, shape (N,)
